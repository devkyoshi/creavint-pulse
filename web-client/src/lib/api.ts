...
function uploadForm<T>(method: "POST" | "PATCH", path: string, form: FormData) {
...
export const api = {
  ...
  articles: {
    upload: (siteId: string, form: FormData) =>
      uploadForm<UploadArticleResponse>("POST", `/sites/${siteId}/articles/upload`, form),
    update: (siteId: string, slug: string, form: FormData) =>
      uploadForm<UploadArticleResponse>("PATCH", `/sites/${siteId}/articles/${slug}/upload`, form),
    saveDraft: (jobId: string, markdown: string, submit = false) =>
      patch<SaveDraftResponse>(`/jobs/${jobId}/draft`, { markdown, submit }),
  },
  review: {
    queue: (siteId?: string) =>
  ...
  analytics: {
    ...
    indexation: (id: string) => get<{ trend: IndexationPoint[] }>(`/analytics/sites/${id}/indexation`),
  },
  alerts: {
